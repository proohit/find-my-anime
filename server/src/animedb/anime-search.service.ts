import { getProviderIdMappings, getProviderSourceMappings } from '@find-my-anime/shared/anime/sources';
import { ADULT_TAGS } from '@find-my-anime/shared/anime/tags';
import { Provider, ProviderDomain } from '@find-my-anime/shared/constants/Provider';
import { LimitedFilterCriteria } from '@find-my-anime/shared/index';
...
export class AnimeSearchService {
    ...
    public async findAnime({
        ...
        const providerDomain = provider ? this.escapeRegExp(ProviderDomain[provider]) : null;
    ...
    private async createProviderMappings() {
        ...
        while (hasMore) {
            const animeBatch = await this.animeModel.find({}).skip(skip).limit(batchSize).exec();
            ...
            Logger.debug(`Updated provider mappings for batch of ${animeBatch.length} anime`);
    ...
    private async createSearchIndexIfNeeded() {
        ...
        const searchIndexExists = indexes.some((index) => index.name === 'titleSynonymsIndex');
